import re, csv, os
directory = r'C:\Users\admin\Documents\Dissertation\Diversity of News\Files\welt'
...
for filename in os.listdir(directory):
    ...
    if filename.endswith(".txt"):
        ...
        with open(filename_dir, 'r', encoding="utf8") as f:
            ...
            with open('C:/Users/admin/Documents/Dissertation/Diversity of News/Helpfiles Skript/welt/redundant_words.csv', newline='') as e:
                    redundantWords = csv.reader(e)
                    regex_before = re.compile('.+Stand:')
                    article = regex_before.sub("",article)
                    regex_after = re.compile('      .+')
                    article = regex_after.sub("",article)
                    #regex_after = re.compile('Quelle:.+')
                    #article = regex_after.sub("",article)
            ...
            article = " ".join(article.split())
            #print(article)
            with open('C:/Users/admin/Documents/Dissertation/Diversity of News/Files/welt/Clean/'+filename,'w', encoding="utf-8") as e:
                e.write(article)
